...
simulate_marketing <- function(parameters, da_prices, id_prices, id_index_names,
  ...
  da_optim <- optimise_schedule(schedule = fixed_schedule,
  ...
  revenues <- sapply(list(no_optim$trades, da_optim$trades, id_optim$trades),
                     calc_revenue)
  ...
  if(simplify){
    data.frame(type = c("constant", "da_optim", "id_optim"),
